...
| `json` | programmatic path | `""` | JSON object with a `plugins` array of per-plugin partial-merge patches, `{ "plugins": [{ "name": string, "url"?, "path"?, "sha"? }] }`. Each entry names an existing plugin and changes only the fields it provides. Validated against the committed [input schema](schemas/2.0/input.json). |
